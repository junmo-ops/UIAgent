...
新版快照记录样式来源类型：原页面内联样式在 B 副本中继续内联，外链样式仍单独加载，保持顺序、media 条件及禁用状态，减少小 CSS 请求。旧快照未记录来源类型时保留原加载方式；更新插件并重新创建副本后生效。服务端对样式清单使用最多 32 个条目、估算字符串大小 32 MiB 的 LRU 缓存，通过文件状态检查失效；此缓存也适用于旧副本，不缓存用户权限或可编辑覆盖样式。
