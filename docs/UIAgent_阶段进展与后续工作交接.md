...
用户反馈多个网站的副本生成验收通过，生成阶段按当前范围收口：B 默认，A 保留回退/诊断；外链依赖与离线能力边界继续保留。这是人工样本验收，不代表历史 G1 实验矩阵全部执行通过。

下一阶段为“在副本上理解需求、修改页面并验证结果”。完整待实施方案见 [副本编辑与真实渲染验证实施方案](UIAgent_副本编辑与真实渲染验证实施方案.md)，包含接口契约、候选版本、浏览器测量、提交门禁、代码落点和验收矩阵。本次只沉淀方案，没有实现新编辑闭环。下文保留历史记录，涉及“仍需重新验证生成”等状态以本节最新反馈为准。
