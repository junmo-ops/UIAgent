...
**技术难点：** 用户操作的是浏览器中已经展示的页面，而不是原系统的开发工程。副本既要保留当前页面的结构和视觉特征，又要便于继续修改，不能只保存一张截图。

**具体设计：** 插件获取页面的 DOM 结构、当前状态和样式信息，服务端组织为独立的 HTML、CSS 文件，并生成页面结构索引和元素映射。样式采用“原规则＋独立覆盖文件”的方式：沿用可获取的原页面样式，将新增调整写入 `author-overrides.css`。原始样式与本轮修改分开保存，便于查找改动和管理版本。

**作用与验证：** 用户无需接入原系统代码仓库，也不必从空白画布重建页面。已有多个真实页面的人工对照及后文案例素材；图片、字体和部分样式仍可能依赖原站资源，不承诺完全离线或任意页面完整还原。

### 2. 结合选中区域和页面结构，让 AI 理解修改对象

**技术难点：** 网页源码和样式可能很大，用户却通常只修改其中一小块。只提供文字指令会缺少页面依据，一次塞入整页源码又会增加无关信息。

**具体设计：** 系统用元素标识（`sourceId`）连接浏览器选区与源码，提供目标元素、所在容器、周边元素和捕获时的布局信息。AI 编辑助手（Agent）可以通过工具按需读取局部源码、查询样式规则，并参考可识别组件的规范。首次写入前，模型必须通过 `declare_intent` 声明目标、相关元素和修改约束；有影响结果的歧义时先询问用户。程序不根据固定文案或网站名称猜测布局。

**作用与验证：** 将“用户指的是哪里”转为可定位、可查询的编辑依据，同时保留继续获取信息的能力。后文案例展示了选区与自然语言结合的使用方式；对耗时和模型调用量的改善仍需同类任务对照，不能仅凭设计推定性能收益。

### 3. 通过限定工具执行修改，并保留可恢复的版本

**技术难点：** 模型返回“修改成功”并不等于页面真的正确。多步操作可能中途失败，直接改写正式版本容易留下半成品。

**具体设计：** 模型通过限定的源码工具修改文字、属性、结构和 CSS。每轮在临时工作副本中执行，系统检查工具参数、源码规则和修改范围，将具体错误反馈给模型；通过提交检查后才形成新版本（Revision），失败或取消则丢弃本轮临时修改。下拉、复选和面板切换等演示交互采用规定格式描述，由统一程序执行，不开放真实业务提交。

**作用与验证：** 将模型规划与程序执行、版本提交分开，使修改有记录、失败可恢复。源码检查并不能证明视觉正确；目前仍需人工确认位置和样式，自动浏览器布局检查属于实验能力，不作为已全面实现的成果。

### 4. 保存页面、对话和版本，支持持续调整方案

**技术难点：** 一次编辑可能需要多轮模型调用，长时间等待单个 HTTP 请求容易遇到网关超时；关闭侧栏、取消任务或回退版本时，页面与对话也需要保持对应。

**具体设计：** 服务端接收任务后返回 HTTP 202 和任务标识，在后台执行编辑；插件通过独立接口查询进度和结果，取消请求通过中止信号传递给执行过程。副本内对话保存到服务端，并与修改版本关联，撤销重做按版本恢复对应内容。日志使用任务与追踪标识关联模型调用、工具操作、错误和耗时，便于区分模型等待与工具执行问题。

**作用与验证：** 避免用一个长连接请求承载整轮编辑，支持重新打开侧栏后查看状态、停止后继续编辑，以及页面和对话的版本恢复。相关流程已有人工验证记录；该机制不等于运行中任务可以跨服务重启续跑，文件跨部署保留仍依赖存储或备份。

### 5. 面向页面需求的专用能力建设

项目建设重点是页面捕获与副本生成、选区和源码映射、页面上下文查询、限定源码工具、演示交互规则、版本与对话管理，以及插件和服务端的协同流程。申报的创新贡献聚焦这些面向页面需求的专用能力，不包含底层模型和通用运行时本身。

技术特色在于把这些能力连接成面向真实网页的编辑流程：模型负责理解需求和规划改动，程序负责可观测的检查、执行状态和版本管理，让生成结果成为可以继续修改的页面方案，而不只是一次文字回复。

【图片占位：技术架构图待更新】

保留“浏览器端—行内应用服务—模型服务”的逻辑结构；Agent Runtime 展示通用运行时职责，不标注具体 SDK 名称。
