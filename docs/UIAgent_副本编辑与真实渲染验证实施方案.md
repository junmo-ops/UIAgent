...
日期：2026-09-06  
状态：历史方案，仅供背景参考。2026-09-19 已按产品决定移除自动候选验证与修正链路，相关开关和接口不再存在；当前仅保留直接版本提交。下文为历史设计及阶段记录，不代表当前实现。
...
最新验收更新（2026-09-06）：用户确认连续三轮编辑与撤销/重做、执行中关闭重开侧栏、停止后继续编辑、执行中重启服务四项场景均无问题。M3 直接提交流程据此收口。上方实施进度中的“未执行浏览器验收或测试”描述此前编码 Agent 的执行情况，不包含本次用户人工验收；记录详见 [阶段进展与后续工作交接](UIAgent_阶段进展与后续工作交接.md)。本次未附新增日志或截图，不代表下文全部验收矩阵通过。
...
当前收口范围：直接提交流程的连续编辑、侧栏重连、停止与服务重启已获用户人工验收通过。下列候选草稿、证据生命周期及验证后提交目标仍属于实验路径后续事项，不计入本次完成范围。

- 恢复异常处理已补充：普通执行与侧栏重连共用进度查询；请求回执不确定或终态后的刷新/对话保存失败时保留任务引用，自动退避重试（最长间隔 15 秒），不重新提交修改请求。任务未确认期间禁止新一轮发送与历史操作；查询到任务丢失后刷新正式副本并提示。用户已验证上述四项场景；独立断网、回执丢失及结果同步失败的故障注入未单独报告验收结果。
