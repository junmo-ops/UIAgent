...
<body>
  ...
  <script>
    ...
    function renderList(){const q=document.querySelector('#filter').value.toLowerCase();const visible=logs.filter(x=>JSON.stringify(x).toLowerCase().includes(q));document.querySelector('#count').textContent='共 '+visible.length+' 条，最多保留 200 条';const root=document.querySelector('#entries');root.replaceChildren(...visible.map(item=>{const b=document.createElement('button');b.className='entry '+(item.id===selectedId?'active':'');b.onclick=()=>show(item.id);const title=document.createElement('div');title.className='entry-title';title.textContent=item.originalInstruction??item.instruction;const meta=document.createElement('div');meta.className='meta';const status=document.createElement('span');status.className='status '+item.status;status.textContent=item.status;meta.append(status,document.createTextNode(new Date(item.timestamp).toLocaleString()+(item.durationMs!=null?' · '+item.durationMs+'ms':'')));b.append(title,meta);return b}))}
    async function show(id){selectedId=id;renderList();const response=await fetch('/v1/logs/'+encodeURIComponent(id));const item=await response.json();const detail=document.querySelector('#detail');detail.replaceChildren();const info=document.createElement('section');info.className='card';info.innerHTML='<h2>基本信息</h2><div class="kv"></div>';const kv=info.querySelector('.kv');[['状态',item.status],['模式',item.kind==='assistant_turn'?'问答 / 技能':'静态源码副本'],['模型',(item.model.provider||'')+' / '+(item.model.name||item.model.mode)],['会话 ID',item.request.editSessionId],['Turn ID',item.request.turnId],['Trace ID',item.request.traceId],['路由 Trace ID',item.request.assistantTraceId??'未记录'],['耗时',item.durationMs==null?'-':item.durationMs+' ms']].forEach(([k,v])=>{const a=document.createElement('div');a.className='key';a.textContent=k;const b=document.createElement('div');b.textContent=esc(v);kv.append(a,b)});detail.append(info);for(const [title,value] of [['用户原文',item.request.originalInstruction??item.request.instruction],['执行指令',item.request.instruction],['工具调用统计',item.toolStatistics?{尝试次数:item.toolStatistics.attempted,成功:item.toolStatistics.succeeded,失败:item.toolStatistics.failed,已拦截:item.toolStatistics.blocked,未完成:item.toolStatistics.pending,参数校验失败:item.toolStatistics.parameterFailures,执行阶段失败:item.toolStatistics.executionFailures}:'未记录'],['当前请求',item.request],['此前对话',item.conversation],['运行诊断',item.codingAgent?.checkpoint?.runtime??item.assistant?.runtime??null],['提交与回滚',item.codingAgent?.checkpoint?.lifecycle??null],[item.kind==='assistant_turn'?'技能工具调用':'源码工具循环',item.assistantSteps??item.sourceSteps??[]],['模型结果',item.result??item.error??null]]){const card=document.createElement('section');card.className='card';const h=document.createElement('h2');h.textContent=title;const pre=document.createElement('pre');pre.textContent=typeof value==='string'?value:JSON.stringify(value,null,2);card.append(h,pre);detail.append(card)}}
