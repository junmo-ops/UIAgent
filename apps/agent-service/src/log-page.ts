...
<body>
  ...
  <script>
    ...
    async function show(id){selectedId=id;renderList();const response=await fetch('/v1/logs/'+encodeURIComponent(id));const item=await response.json();const detail=document.querySelector('#detail');detail.replaceChildren();const info=document.createElement('section');info.className='card';info.innerHTML='<h2>基本信息</h2><div class="kv"></div>';const kv=info.querySelector('.kv');[['状态',item.status],['模式',item.sourceWorkspaceId?'静态源码副本':'直接 DOM'],['模型',(item.model.provider||'')+' / '+(item.model.name||item.model.mode)],['会话 ID',item.request.editSessionId],['Turn ID',item.request.turnId],['Trace ID',item.request.traceId],['耗时',item.durationMs==null?'-':item.durationMs+' ms']].forEach(([k,v])=>{const a=document.createElement('div');a.className='key';a.textContent=k;const b=document.createElement('div');b.textContent=esc(v);kv.append(a,b)});detail.append(info);for(const [title,value] of [['当前请求',item.request],['此前对话',item.conversation],['源码工具循环',item.sourceSteps??[]],['模型调用',item.modelAttempts??[]],['模型结果',item.result??item.error??null],['执行、观察与验证',item.executions??[]]]){const card=document.createElement('section');card.className='card';const h=document.createElement('h2');h.textContent=title;const pre=document.createElement('pre');pre.textContent=JSON.stringify(value,null,2);card.append(h,pre);detail.append(card)}}
