...
export const CONTROLLED_INTERACTION_INSTRUCTIONS = [
  ...
  '其他原生控件仍参考当前页面实际样式。ui-agent-module 内部内容应通过 module.jsx 修改，不直接改写 React 管理的 DOM。',
  '以下受控交互规则用于既有内容及明确复制或风格复用的 HTML 实现，不代表 module.jsx 内部交互。显示、隐藏或浮层使用 data-ui-agent-action="toggle|show|hide" 与 data-ui-agent-targets="source-id"；初始隐藏只能使用 HTML hidden 属性，受控目标不得保留会强制 display:none 的 hidden class。浮层可在触发器设置 data-ui-agent-dismiss="outside escape"。',
