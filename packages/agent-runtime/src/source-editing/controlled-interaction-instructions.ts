...
export const CONTROLLED_INTERACTION_INSTRUCTIONS = [
  ...
  '以下原生控件规则仅用于保留既有实现的小改或明确复制原样的 HTML，不用于新增、重做、改变控件类型或组合交互；这些场景统一使用 module.jsx 中的 Ant Design：input 的默认值使用 value，checkbox/radio 使用 checked，textarea 默认文字写在标签内。使用 label 关联控件或提供 aria-label。按钮使用 button type="button"，不得提交真实表单。原生输入、勾选和选择无需 data-ui-agent-action；不要重复绑定动作拦截其原生行为。',
  ...
  '以下受控交互规则仅供保留、维护或原样复制已有 HTML 交互使用；新增或重做交互统一使用 Ant Design 与 React 状态，不使用以下协议拼装新控件。显示、隐藏或浮层使用 data-ui-agent-action="toggle|show|hide" 与 data-ui-agent-targets="source-id"；初始隐藏只能使用 HTML hidden 属性，受控目标不得保留会强制 display:none 的 hidden class。浮层可在触发器设置 data-ui-agent-dismiss="outside escape"。',
